...
public class SetWarpCommand implements CommandExecutor {
    ...
    public SetWarpCommand(MPEssentials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(plugin.parseMessage("<red>Usage: /setwarp <name></red>"));
            return true;
        }

        plugin.getWarpService().setWarp(args[0], player.getLocation());
        player.sendMessage(plugin.parseMessage(config.getMessage("warp-set")));
        return true;
    }
}
